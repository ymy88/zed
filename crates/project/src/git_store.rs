        ...
        if let BufferDiffEvent::HunksStagedOrUnstaged(new_index_text) = event {
            ...
            if let Some(diff_state) = self.diffs.get(&buffer_id) {
                ...
                if let Some((repo, path)) = self.repository_and_path_for_buffer_id(buffer_id, cx) {
                    let new_index_for_recalc = new_index_text.clone();
...
impl Repository {
                ...
                RepositoryState::Remote(RemoteRepositoryState {
                    ...
                    Ok(git::repository::FileHistory {
                        entries: response
                            ...
                            .collect(),
                        path: git::repository::RepoPath::from_proto(&response.path)?,
                    })
                }
            }
        })
    ...
    pub fn branch_log(
        ...
        let id = self.id;
        self.send_job("branch_log", None, move |git_repo, _cx| async move {
            match git_repo {
                RepositoryState::Local(LocalRepositoryState { backend, .. }) => {
    ...
    pub fn diff_name_status(
        ...
        let id = self.id;
        self.send_job("diff_name_status", None, move |git_repo, _cx| async move {
            match git_repo {
                RepositoryState::Local(LocalRepositoryState { backend, .. }) => {
    ...
    pub fn diff_file_text(
        ...
        let id = self.id;
        self.send_job("diff_file_text", None, move |git_repo, _cx| async move {
            match git_repo {
                RepositoryState::Local(LocalRepositoryState { backend, .. }) => {
    ...
    pub fn parent_branch(
        ...
        self.send_job("parent_branch", None, move |repo, _| async move {
            match repo {
                RepositoryState::Local(LocalRepositoryState { backend, .. }) => {
